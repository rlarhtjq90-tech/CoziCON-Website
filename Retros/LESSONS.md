...
### 공개 레이아웃 컴포넌트도 세션을 확인해야 한다 #coding #next-auth #ux
랜딩 GNB처럼 미인증 경로에 사용되는 헤더가 세션 체크 없이 항상 "로그인" 버튼을 보여주면, 로그인된 사용자가 대시보드→메인으로 돌아올 때 로그아웃된 것처럼 보이는 UX 버그가 발생함.
`useSession()`으로 loading/authenticated/unauthenticated 세 상태를 분기하면 실제 로그아웃 없이 올바른 UI를 보여줄 수 있음. `SessionProvider`가 이미 전체 앱을 감싸고 있는지 먼저 확인할 것.

### JWT에 파생 데이터를 넣으면 페이지마다 DB 재조회를 없앨 수 있다 #coding #next-auth #performance
`companyName` 같이 로그인 이후 자주 표시하는 파생 데이터는 `authorize()`에서 한 번 조회해 JWT에 담아두면, 이후 모든 페이지에서 `session.user.companyName`으로 DB 없이 접근 가능.
단, JWT는 재로그인 전까지 값이 갱신되지 않으므로 회사명 변경 시 재로그인이 필요한 트레이드오프가 있음.
